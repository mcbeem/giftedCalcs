...
plot.perf_by_cutoff <- function(x, colors = c("#377eb8", "#e41a1c"),
  ...
  # check inputs
  if (!is.logical(show.optimal)) {
    stop("Argument show.optimal must be TRUE or FALSE")
  }
  if (!is.logical(show.cost)) {
    stop("Argument show.cost must be TRUE or FALSE")
  }
  if (min(areColors(colors)) != 1) {
    stop("Argument colors must be a vector of two valid colors")
  }
  if (length(colors) != 2) {
    stop("Argument colors must be a vector of two valid colors")
  }
  ...
  if (show.cost == FALSE) {
    p <- ggplot2::ggplot(
      data = long[long$parameter == "sensitivity", ],
  ...
  if (show.optimal == TRUE & show.cost == TRUE) {
    p <- p + ggplot2::geom_point(
      ...
      ggplot2::geom_hline(
        yintercept = bestvalues$sensitivity,
        alpha = .7, linetype = "dashed", color = colors[1]
      ) +
      ggplot2::geom_hline(
        yintercept = bestvalues$cost,
