...
expect_equal(p_identified(relyt=.9, x=1, test.cutoff=.9,
...
expect_error(p_identified(relyt=.9, x=1, test.cutoff=.9,
                          nom.cutoff=.9, valid=.99, mu=0))

expect_error(p_identified(relyt=.9, test.cutoff=.9, blarg=4))

expect_warning(p_identified(relyt=.9, x=1, test.cutoff=.9,
                          nom.cutoff=.9, mu=-1))

expect_warning(p_identified(relyt=.9, x=1, test.cutoff=.9,
                          valid=.6, mu=-1))

expect_error(p_identified(relyt=.9, x=1, test.cutoff=1,
                          valid=.6, mu=-1))
