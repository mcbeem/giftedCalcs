...
expect_equal(d_identified_v(relyt=.9, x=1, test.cutoff=.9,
...
expect_error(d_identified_v(relyt=.9, x=1, test.cutoff=.9,
...
expect_warning(d_identified_v(relyt=.9, x=1, test.cutoff=.9,
                          nom.cutoff=.9, mu=-1, normalize=T))

expect_warning(d_identified_v(relyt=.9, x=1, test.cutoff=.9,
                          valid=.6, mu=-1, normalize=T))

expect_error(d_identified_v(relyt=.9, x=1, test.cutoff=1,
                          valid=.6, mu=-1, normalize=T))
